...
25、如何实现在`Django`项目中配置静态文件目录`static`
...
69、什么是链式查询
70、`ORM`模糊查询__支持哪6种语法，他们都表示什么意思，针对时间列有哪2种模糊查询语法
...
80、`ORM`如何实现外键列，外键列生成后是什么样的，如何实现多对多关系，一对一关系
...
81、创建一条记录时，一对多关系的外键列如何指定列值，有哪两种方式，如何通过外键查询
...
```
会自动生成，through是指定多对多关系的表名，through_filed表示多对多关系的约束列
```
...
90、如何在分组查询时，使用聚合函数，分组查询时，会打印分组的列值吗，如何展示结果
...
```
一样，在官方文档中有时会遇到Book.objects.all().annotate()，都包含主键
```
...
Please select a fix:
 ...
 # 选择1，系统提示Please enter the default value now, as valid Python：请为新增字段添加一个默认值，输入一个默认值>>> 0，后确定，再执行python3 manage.py migrate即成功添加列
```
...
```python
# 如果没有添加ectype，那么在request.FILES中无法
