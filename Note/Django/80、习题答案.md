...
{% for page_num in paginator.page_range{% for page_number in paginator.page_range %}
...
125、如果在`request.POST`的请求数据中不存储`forms`校验类中的字段，那么`is_valid()`方法的返回值是什么
...
# 创建用户表时，可以继承于该类，该类本身就包含`username`、`email`等，所以继承该类后，就无需创建继承类中包含的字段
